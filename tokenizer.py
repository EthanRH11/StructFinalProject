...
patterns = [
    [r"=>", "arrow"], # arrow operator for lambda functions
    [r"lambda", "lambda"], # lambda keyword
...
def tokenize(characters, generated_tags=test_generated_tags):
    ...
    while position < len(characters):
        ...
        # Count newlines in multiline comments to track line numbers correctly
        if tag == "comment":
            for c in value:
                if c == "\n":
                    line = line + 1
...
def test_comments():
    ...
    assert verify_same_tokens("/* this is a comment */", "\n")
    assert verify_same_tokens("/* multi-line \n comment */", "\n")
    assert verify_same_tokens("1 + 2 /* comment */", "1 + 2 \n")
    assert verify_same_tokens("/* comment */ 3 + 4", "\n 3 + 4")
    assert verify_same_tokens("5 /* comment */ + /* another */ 6", "5  \n +  \n 6")
...
def test_multiline_comments():
    ...
    t = remove_line_info(tokenize("/* this is a \n multiline comment */ 1 + 2;"))
    t1 = remove_line_info(tokenize("/* this is a single line comment, but should be treated the same */ 2 + 4;"))
    tags = [tok["tag"] for tok in t[:-1]]
    assert tags == ["number", "+", "number", ";"], f"got {tags}"

def test_lambda_and_arrow():
    print("testing lambda and arrow tokens...")
    t = remove_line_info(tokenize("lambda x => x + 1"))
    tags = [tok["tag"] for tok in t[:-1]]
    assert tags == ["lambda", "identifier", "arrow", "identifier", "+", "number"], f"got {tags}"
